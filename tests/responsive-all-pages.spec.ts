...
async function checkElementResponsive(page: Page, selector: string, viewportName: string) {
  ...
  if (box && viewport) {
    ...
    if (isOverflowing) {
      console.error(`❌ [${viewportName}] Element overflows: ${selector} (width: ${box.width}px, viewport: ${viewport.width}px)`)
    }
      ...
      if (await element.isVisible()) {
        ...
        if (box) {
          ...
          if (isTooSmall) {
            ...
            console.warn(`⚠️  Element too small for touch: "${text?.substring(0, 30)}" (${box.width}x${box.height}px)`)
          }
        }
      }
    }
