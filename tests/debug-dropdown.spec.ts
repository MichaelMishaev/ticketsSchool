...
test.describe('Debug: Dropdown Button Rendering', () => {
  ...
  test('STEP 5: Detailed check - Regular ADMIN with school', async ({ page }) => {
    ...
    const state = {
      ...
      hasLoadingText: await page.locator('text=טוען...').isVisible().catch(() => false),
      hasDashboard: await page.locator('h2:has-text("לוח בקרה")').isVisible().catch(() => false),
      hasDropdownButton: await page.locator('button[aria-label="צור אירוע חדש"]').isVisible().catch(() => false),
    }
