...
const uniqueEmail = (prefix: string) =>
  `${prefix}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}@test.com`

test.describe('Basic kartis.info Tests', () => {
  test.afterAll(async () => {
  ...
  test('mobile responsive - hamburger menu', async ({ page }) => {
    ...
    const navVisible = await page
      .locator('text=ראשי, text=אירועים, text=Dashboard, text=Events')
      .first()
      .isVisible()
      .catch(() => false)
