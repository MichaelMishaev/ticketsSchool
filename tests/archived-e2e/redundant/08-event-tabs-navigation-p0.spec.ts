...
test.describe('Event Tabs Navigation P0 - Critical Tests', () => {
  ...
  test.describe('[08.1] Tab Switching', () => {
    ...
    test('should show active tab indicator (pulsing dot)', async ({ page }) => {
      ...
      const activeIndicator = page.locator(
        '[role="tab"][aria-selected="true"]:visible .animate-ping'
      )
      ...
      const newActiveIndicator = page.locator(
        '[role="tab"][data-tab-id="registrations"]:visible .animate-ping'
      )
      await expect(newActiveIndicator).toBeVisible()

      // Previous tab should NOT have indicator
      const previousTabIndicator = page.locator(
        '[role="tab"][data-tab-id="overview"]:visible .animate-ping'
      )
  ...
  test.describe('[08.2] URL Parameter Updates', () => {
    test('should update URL with ?tab parameter when switching tabs', async ({ page }) => {
      ...
      // Click Registrations tab
      await page
        .locator('[role="tab"][data-tab-id="registrations"]:visible')
      ...
      // Click Check-In tab
      await page
        .locator('[role="tab"][data-tab-id="checkin"]:visible')
        .evaluate((el: any) => el.click())
      ...
      // Click Reports tab
      await page
        .locator('[role="tab"][data-tab-id="reports"]:visible')
        .evaluate((el: any) => el.click())
      await page.waitForTimeout(500)
      ...
      // Click Overview tab
      await page
        .locator('[role="tab"][data-tab-id="overview"]:visible')
        .evaluate((el: any) => el.click())
      await page.waitForTimeout(500)
  ...
  test.describe('[08.4] Tab Content Loading', () => {
    test('should load Overview tab content correctly', async ({ page }) => {
      ...
      await expect(page.locator('[role="tab"][data-tab-id="overview"]:visible')).toHaveAttribute(
        'aria-selected',
        'true'
      )
    ...
    test('should load Registrations tab content correctly', async ({ page }) => {
      ...
      await expect(
        page.locator('button:has-text("הכל"), button:has-text("מאושרים")').first()
    ...
    test('should load Check-In tab content correctly', async ({ page }) => {
      ...
      await expect(page.locator('text=/מאושרים|המתנה|ביטולים/i').first()).toBeVisible({
        timeout: 10000,
      })
    })
    ...
    test('should load Reports tab content correctly', async ({ page }) => {
      ...
      await expect(page.locator('text=/דוחות וסטטיסטיקות|ייצוא/i').first()).toBeVisible({
        timeout: 10000,
      })
    })
  })
