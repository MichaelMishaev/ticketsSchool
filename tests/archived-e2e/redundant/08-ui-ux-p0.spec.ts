...
test.describe('UI/UX & Accessibility P0 - Critical Tests', () => {
  ...
  test.describe('[08.2.1-2.4] Hebrew RTL Layout', () => {
    test('page has RTL direction for Hebrew content', async ({ page }) => {
      ...
      const hasRtl = htmlDir === 'rtl' || (await page.locator('[dir="rtl"]').count()) > 0
  ...
  test.describe('[08.4.6] Error Feedback', () => {
    test('error message shown for invalid form submission', async ({ page }) => {
      ...
      const hasDisabledButton = await page
        .locator('button:disabled:has-text("נא למלא")')
        .isVisible()
        .catch(() => false)
      const hasErrorList = await page
        .locator('text=/יש למלא|missing|required/i')
        .isVisible()
        .catch(() => false)
  ...
  test.describe('[08.5.2] Required Field Indicators', () => {
    test('required fields are marked', async ({ page }) => {
      ...
      const hasAsterisk = await page
        ...
        .catch(() => false)
      const hasRequiredText = await page
        .locator('text=/חובה|required/i')
        .isVisible()
        .catch(() => false)
      const hasRequiredAttr = (await page.locator('input[required]').count()) > 0
      const hasDisabledSubmit = await page
        .locator('button:has-text("נא למלא")')
        .isVisible()
        .catch(() => false)
