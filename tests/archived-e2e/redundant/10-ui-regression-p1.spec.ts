...
test.describe('UI/UX Regression P1 - User-Facing Bugs', () => {
  ...
  test.describe('Bug #0: TABLE_BASED Events Show "No Spots Available"', () => {
    test('should show event as open when tables are available', async ({ page }) => {
      ...
      const isOpenVisible = await page
        .locator('text=/פתוח|Open/i')
        .isVisible()
        .catch(() => false)
    ...
    test('should show correct submit button text for table-based events', async ({ page }) => {
      ...
      const hasBookingButton = await page
        .locator('button:has-text(/אשר הזמנה/i)')
        .isVisible()
        .catch(() => false)
      const hasWaitlistButton = await page
        .locator('button:has-text(/רשימת המתנה/i)')
        .isVisible()
        .catch(() => false)
  ...
  test.describe('Bug #19: Missing Form Validation - Submit Button Active Without Required Fields', () => {
    test('should disable submit button when required fields are missing', async ({ page }) => {
      ...
      if (!isDisabled) {
        ...
        const hasErrors = await page
          .locator('text=/נא למלא|שדה חובה|required/i')
          .isVisible()
          .catch(() => false)
    ...
    test('should show missing field errors when clicking submit with empty form', async ({
      page,
    }) => {
      const school = await createSchool()
      ...
      if (!isDisabled) {
        ...
        const hasErrors = await page
          .locator('text=/נא למלא|שדה חובה|שדות חובה|required|missing/i')
  ...
  test.describe('Bug #20: Admin Panel Displaying Field IDs Instead of Labels', () => {
    test('should show field labels (not IDs) in registration data', async ({ page }) => {
      ...
      const hasFieldIds = await page
        .locator('text=/field_[0-9a-f]+/i')
        .isVisible()
        .catch(() => false)
  ...
  test.describe('Bug #22: Event Overview Shows "No Participants" Despite Active Registrations', () => {
    test('should show registrations in recent activity section when participants exist', async ({
      page,
    }) => {
      ...
      const hasActivity = await page
        .locator('text=Active User')
        .isVisible({ timeout: 5000 })
        .catch(() => false)

      // Should NOT show "טרם נרשמו משתתפים" (no participants registered yet)
      const hasNoParticipantsMessage = await page
        .locator('text=/טרם נרשמו משתתפים/i')
        .isVisible()
        .catch(() => false)
    ...
    test('should show consistent participant count between stats and activity', async ({
      page,
    }) => {
      ...
      const statsText = await page
        .locator('text=/2.*מאושרים|מאושרים.*2/i')
        .isVisible()
        .catch(() => false)

      // Check recent activity shows registrations (not "no participants")
      const hasNoParticipants = await page
        .locator('text=/טרם נרשמו משתתפים/i')
        .isVisible()
        .catch(() => false)
  ...
  test.describe('Bug #9: Create Event Dropdown Clipped on Desktop', () => {
    test('dropdown should be fully visible on desktop viewport', async ({ page }) => {
      ...
      if (isVisible) {
        ...
        const dropdown = page
          .locator('[role="menu"], .dropdown-menu, div[class*="dropdown"]')
          .first()
