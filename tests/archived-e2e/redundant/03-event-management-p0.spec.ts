...
test.describe('Event Management P0 - Critical Tests', () => {
  ...
  test.describe('[03.1.3] Payment Settings UX (Bug Fix)', () => {
    test('when payment required is checked, pricing model should not show "Free" option', async ({
      page,
    }) => {
    ...
    test('when payment required is unchecked, pricing model should default to Free', async ({
      page,
    }) => {
      ...
      const paymentCheckbox = page.locator(
        'input[type="checkbox"]:near(:text("דרוש תשלום לאירוע זה"))'
      )
