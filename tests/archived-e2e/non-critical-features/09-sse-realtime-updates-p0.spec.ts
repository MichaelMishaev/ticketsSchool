...
test.describe.skip('SSE Real-Time Updates P0 - Critical Tests', () => {
  ...
  test.describe('[09.1] SSE Connection Establishment', () => {
    test('should establish SSE connection when Registrations tab is opened', async ({ page }) => {
      ...
      const hasConnectionMessage = consoleMessages.some(
        (msg) => msg.includes('SSE connected') || msg.includes('SSE connection established')
      )
    ...
    test('should show connection indicator when SSE is connected', async ({ page }) => {
      ...
      const connectionIndicator = page.locator(
        'text=/עדכונים בזמן אמת|real.*time|connected/i, [data-testid="connection-status"]'
      )
  ...
  test.describe('[09.2] Real-Time Registration Updates', () => {
    test('should show new registration in admin dashboard within 2-4 seconds', async ({
      page,
      browser,
    }) => {
      // Setup
    ...
    test('should display toast notification when new registration arrives', async ({
      page,
      browser,
    }) => {
      // Setup
      ...
      await expect(
        page.locator(`text=/Toast Test User.*נרשם|registered.*Toast Test User/i`)
  ...
  test.describe('[09.4] Connection Resilience', () => {
    test('should auto-reconnect if SSE connection is lost', async ({ page }) => {
      ...
      const hasReconnect = consoleMessages.some(
        (msg) => msg.includes('Reconnecting') || msg.includes('SSE connected')
      )
    ...
    test('should show disconnected indicator when connection is lost', async ({ page }) => {
      ...
      const disconnectIndicator = page.locator(
        'text=/לא מחובר|disconnected|מנותק/i, [data-testid="connection-status"][data-connected="false"]'
      )
  ...
  test.describe('[09.5] Multiple Admins Scenario', () => {
    test('should send updates to multiple admins simultaneously', async ({ page, browser }) => {
      ...
      const admin1InitialCount = await page
        .locator('[data-testid="registration-row"], .registration-card, .registration-item')
        .count()
      const admin2InitialCount = await admin2Page
        .locator('[data-testid="registration-row"], .registration-card, .registration-item')
        .count()
      ...
      const admin1NewCount = await page
        .locator('[data-testid="registration-row"], .registration-card, .registration-item')
        .count()
      const admin2NewCount = await admin2Page
        .locator('[data-testid="registration-row"], .registration-card, .registration-item')
        .count()
