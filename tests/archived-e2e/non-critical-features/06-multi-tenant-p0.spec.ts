...
test.describe('Multi-Tenancy P0 - CRITICAL Security', () => {
  ...
  test.describe('[06.1.1-1.6] Event Isolation', () => {
    ...
    test('School A admin cannot access School B event by direct URL', async ({ page }) => {
      ...
      const hasError = await page
        .locator('text=/403|404|לא נמצא|אין הרשאה/i')
        .isVisible()
        .catch(() => false)
    ...
    test('API /api/events returns only current school events', async ({ page, request }) => {
      ...
      const schoolAEvent1 = await createEvent()
        .withTitle('School A Event 1')
        .withSchool(schoolA.id)
        .create()
      const schoolAEvent2 = await createEvent()
        .withTitle('School A Event 2')
        .withSchool(schoolA.id)
        .create()
    ...
    test('Cannot create event with manipulated schoolId in request body', async ({
      page,
      request,
    }) => {
  ...
  test.describe('[06.2.1-2.5] Registration Isolation', () => {
    ...
    test('Cannot export other school registrations', async ({ page, request }) => {
      ...
      const schoolBEvent = await createEvent()
        .withTitle('School B Export Event')
        .withSchool(schoolB.id)
        .create()
  ...
  test.describe('[REGRESSION] Bug #11: Data Isolation Bypass - Session After Onboarding', () => {
    test('session should be updated immediately after onboarding completes', async ({ page }) => {
      // This tests the fix for Bug #10 and #11
      // After onboarding, session cookie MUST contain schoolId
      const admin = await createAdmin()
        .withEmail(generateEmail('onboarding-session'))
        .withPassword('TestPassword123!')
        .withSchool(null) // No school initially
        .create()

      // Simulate onboarding by creating a school and updating admin
      const school = await createSchool().withName('Onboarding Session School').create()

      // Update admin with school
      await prisma.admin.update({
        where: { id: admin.id },
        data: { schoolId: school.id },
      })

      // Login (should have schoolId in session now)
      const loginPage = new LoginPage(page)
      await loginPage.goto()
      await loginPage.login(admin.email, 'TestPassword123!')
      ...
      await page.fill('input[name="title"]', 'Post Onboarding Event')
      await page.fill('input[name="slug"]', `post-onboarding-${Date.now()}`)
      ...
      if (isVisible) {
        ...
        // Event should be created successfully (session has schoolId)
        ...
        await page.goto('/admin/events')
        await expect(page.locator('text=Post Onboarding Event')).toBeVisible({ timeout: 5000 })
      }
    })
  })

  test.describe('[06.7.1-7.3] API Authorization Patterns', () => {
    test('API always enforces schoolId from session, not request body', async ({
      page,
      request,
    }) => {
