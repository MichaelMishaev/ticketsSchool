...
test.describe('Ban Enforcement System (P0)', () => {
  ...
  test('should block banned user from registering (game-based ban)', async ({ page }) => {
    // Setup
    const school = await createSchool().withName(generateSchoolName()).create()
    const event = await createEvent().withSchool(school.id).withTitle('Soccer Match').create()
  ...
  test('should allow registration for expired date-based ban', async ({ page }) => {
    // Setup
    const school = await createSchool().withName(generateSchoolName()).create()
    const event = await createEvent().withSchool(school.id).withTitle('New Event').create()
  ...
  test('should allow registration for completed game-based ban', async ({ page }) => {
    // Setup
    const school = await createSchool().withName(generateSchoolName()).create()
    const event = await createEvent().withSchool(school.id).withTitle('Soccer Final').create()
  ...
  test('should enforce school-level ban isolation', async ({ page }) => {
    ...
    const school1 = await createSchool()
      .withName(`School 1 ${uniqueId}`)
      .withSlug(`school1-${uniqueId}`)
      .create()
    const school2 = await createSchool()
      .withName(`School 2 ${uniqueId}`)
      .withSlug(`school2-${uniqueId}`)
      .create()
