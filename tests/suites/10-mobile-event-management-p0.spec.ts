...
test.describe.skip('Mobile Event Management P0 - Critical Tests', () => {
  ...
  test.describe('[10.1] Bottom Sheet - Mobile Registration Actions', () => {
    ...
    test('should open bottom sheet when registration card is tapped on mobile', async ({ page }) => {
      // Setup
      ...
      const registrationCard = page.locator('[data-testid="registration-row"], .registration-card, .registration-item').first()
      await registrationCard.click()

      // Bottom sheet should appear
      const bottomSheet = page.locator('[data-testid="bottom-sheet"], .bottom-sheet, [role="dialog"]')
    ...
    test('should show all action buttons in bottom sheet', async ({ page }) => {
      ...
      const viewDetailsButton = page.locator('button:has-text("פרטים"), button:has-text("Details"), text=/פרטים מלאים|view.*details/i')
      ...
      const hasCancelOrDelete = await cancelButton.isVisible().catch(() => false) || await deleteButton.isVisible().catch(() => false)
    ...
    test('should close bottom sheet when backdrop is tapped', async ({ page }) => {
      ...
      if (hasBackdrop) {
        ...
        const closeButton = page.locator('button[aria-label*="סגור"], button[aria-label*="close"], button:has-text("✕")')
    ...
    test('should perform action from bottom sheet (confirm waitlist)', async ({ page }) => {
      ...
      const confirmButton = page.locator('button:has-text("אשר"), button:has-text("Confirm"), button:has-text("קבל")')
  ...
  test.describe('[10.2] Floating Action Button (FAB)', () => {
    ...
    test('should show FAB on mobile for export action', async ({ page }) => {
      ...
      if (!hasFAB) {
        const exportButton = page.locator('button:has-text("ייצוא"), button:has-text("Export")').first()
    ...
    test('should open export menu when FAB is tapped', async ({ page }) => {
      ...
      const hasExportMenu = await page.locator('text=/CSV|Excel|PDF/i').isVisible().catch(() => false)
    ...
    test('should have minimum 56px size for touch target', async ({ page }) => {
      ...
      const fab = page.locator('[data-testid="fab"], button:has-text("ייצוא"), button:has-text("Export")').first()
      const box = await fab.boundingBox()
  ...
  test.describe('[10.3] Responsive Behavior - Mobile vs Desktop', () => {
    ...
    test('should show desktop action buttons on desktop', async ({ page }) => {
      ...
      const actionButtons = page.locator('button:has-text("בטל"), button:has-text("אשר"), button:has-text("מחק")')
  ...
  test.describe('[10.4] Touch Interactions', () => {
    ...
    test('should have proper touch target sizes (min 44px)', async ({ page }) => {
      ...
      const buttons = page.locator('[data-testid="bottom-sheet"] button, .bottom-sheet button, [role="dialog"] button')
