  ...
  test('Login and navigate through all steps', async ({ page }) => {
    ...
    if (continueButtonCount > 0) {
      ...
      if (successVisible) {
        ...
        await page.screenshot({ path: 'playwright-report/07-ERROR-premature-success.png', fullPage: true })
        ...
        await page.screenshot({ path: 'playwright-report/07-ERROR-no-step-4-content.png', fullPage: true })
      ...
      await page.screenshot({ path: 'playwright-report/06-ERROR-no-continue-button.png', fullPage: true })
