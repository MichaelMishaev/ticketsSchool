...
test.describe('Registration Edge Cases - CRITICAL', () => {
  ...
  test.describe('Israeli Phone Number Normalization', () => {
    ...
    test('accepts phone with spaces: 050 123 4567', async ({ page }) => {
      ...
      await expect(page.locator('text=הרשמה הושלמה בהצלחה, text=confirmation')).toBeVisible({ timeout: 5000 })
    ...
    test('rejects invalid phone: too short', async ({ page }) => {
      ...
      // Should show error
      await expect(page.locator('text=מספר טלפון לא תקין, text=invalid phone')).toBeVisible({ timeout: 3000 })
  ...
  test.describe('Required Field Validation', () => {
    ...
    test('cannot submit without name', async ({ page }) => {
      ...
      await expect(nameInput).toBeFocused().catch(() => true)

      // Or check for error message
      const hasError = await page.locator('text=נא למלא, text=שדה חובה, text=required').isVisible().catch(() => false)

      expect(hasError || await nameInput.evaluate(el => (el as HTMLInputElement).checkValidity()) === false).toBeTruthy()
    ...
    test('can submit without email if not required', async ({ page }) => {
      ...
      const succeeded = await page.locator('text=הרשמה הושלמה בהצלחה').isVisible({ timeout: 5000 }).catch(() => false)
  ...
  test.describe('Duplicate Registration Prevention', () => {
    ...
    test('prevents duplicate registration with same phone', async ({ page }) => {
      ...
      await expect(page.locator('text=כבר נרשמת, text=already registered')).toBeVisible({ timeout: 5000 })
  ...
  test.describe('Special Characters and Encoding', () => {
    ...
    test('sanitizes potential XSS in name field', async ({ page }) => {
      ...
      const hasScriptTag = await page.locator('script:has-text("alert")').isVisible().catch(() => false)
  ...
  test.describe('Boundary Values', () => {
    ...
    test('handles zero spots request gracefully', async ({ page }) => {
      ...
      if (await spotsInput.isVisible()) {
        ...
        const hasError = await page.locator('text=לפחות מקום אחד, text=at least 1').isVisible({ timeout: 3000 }).catch(() => false)
  ...
  test.describe('Form State and UX', () => {
    ...
    test('shows loading state during submission', async ({ page }) => {
      ...
      const hasLoadingText = await page.locator('text=שולח, text=loading, text=...').isVisible({ timeout: 1000 }).catch(() => false)
      const hasSpinner = await page.locator('.animate-spin, .spinner').isVisible({ timeout: 1000 }).catch(() => false)
