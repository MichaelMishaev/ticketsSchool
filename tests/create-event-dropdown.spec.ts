...
test.describe('CreateEventDropdown - Mobile & Desktop UI/UX Tests', () => {
  ...
  test.describe('Mobile Viewport (375px - iPhone SE)', () => {
    ...
    test('no horizontal scroll after opening dropdown on mobile', async ({ page }) => {
      ...
      // Open dropdown
      await page.click('button[aria-label="צור אירוע חדש"]')
      await page.waitForTimeout(300)
    ...
    test('both event options are visible and clickable on mobile', async ({ page }) => {
      ...
      const restaurantOption = page.locator('a[href="/admin/events/new-restaurant"]:has-text("אירוע מסעדה")')
    ...
    test('clicking restaurant event option navigates correctly', async ({ page }) => {
      ...
      await page.click('a[href="/admin/events/new-restaurant"]:has-text("אירוע מסעדה")')
  ...
  test.describe('Events Page Dropdown (Page Variant)', () => {
    ...
    test('page variant shows event type descriptions', async ({ page }) => {
      ...
      await expect(page.locator('text=/מושלם לאירועי ספורט/i')).toBeVisible()
      await expect(page.locator('text=/ניהול שולחנות והזמנות/i')).toBeVisible()

      // Check for feature badges
      await expect(page.locator('text=/מגבלת קיבולת/i')).toBeVisible()
      await expect(page.locator('text=/ניהול שולחנות/i')).toBeVisible()
    })
  })
  ...
  test.describe('RTL Support', () => {
    test('dropdown aligns correctly in RTL layout', async ({ page }) => {
      ...
      const dropdownMenu = page.locator('[role="menu"]')
      const menuBox = await dropdownMenu.boundingBox()
      const button = page.locator('button[aria-label="צור אירוע חדש"]')
