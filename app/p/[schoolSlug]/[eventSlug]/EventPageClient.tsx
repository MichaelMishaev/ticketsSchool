/**
 * @LOCKED
 * Reason: Business-critical public registration UI (client component)
 * Scope:
...
const RegistrationSuccess = dynamic(() => import('./RegistrationSuccess'), { ssr: false })
...
export default function EventPageClient({ event }: EventPageClientProps) {
  const schoolSlug = event.schoolSlug
  const eventSlug = event.eventSlug

  const [submitting, setSubmitting] = useState(false)
  const [formData, setFormData] = useState<any>(() => {
    const initialData: any = {}
    if (event.fieldsSchema && Array.isArray(event.fieldsSchema)) {
  ...
  // Accessibility popup (mobile only)
  ...
  const gradientFrom = `${schoolColor}40`
  const gradientTo = `${schoolColor}25`
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto px-4">
        {/* Event Header — Hero (school branding merged in) */}
        <div className={`${theme.sectionCard} overflow-hidden mb-6 animate-fadeIn`}>
          ...
          <div className="relative h-56 sm:h-72">
            ...
            {/* School logo avatar overlay */}
            {event.school && (
              <div className="absolute top-4 right-4 z-10 flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-full px-3 py-1.5 shadow-md">
                {event.school.logo ? (
                  <img
                ...
                <span className="text-sm font-semibold text-gray-800">{event.school.name}</span>
              </div>
            )}
          ...
          {(() => {
            ...
            return (
              <div className="px-5 py-4 flex flex-wrap gap-2 border-b border-gray-100" dir="rtl">
                ...
                {/* Social proof: registration count badge */}
                {event._count.registrations >= 3 && (
                  <span className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium bg-purple-50 text-purple-700">
                    <Users className="w-3.5 h-3.5 flex-shrink-0" />
                    {event._count.registrations} משפחות נרשמו
                  </span>
                )}
              </div>
            )
            ...
            {event.eventType === 'CAPACITY_BASED' && (
              <div className="pt-2">
                <div
                  ...
                  className="w-full rounded-full h-3 overflow-hidden bg-gray-100"
                >
                  <div
                ...
                <p
                  className="text-sm font-medium mt-1.5 text-left text-gray-600"
                  aria-hidden="true"
                >
        ...
        <div className={`${theme.sectionCard} p-6 sm:p-8 mb-6 animate-slideUp`}>
          <div className="flex items-center gap-3 mb-6" dir="rtl">
            <div className="w-1.5 h-8 rounded-full" style={{ backgroundColor: schoolColor }} />
            <ClipboardList className="w-5 h-5" style={{ color: schoolColor }} />
            <h2 className={`text-xl font-bold ${theme.cardText}`}>
              {event.eventType === 'TABLE_BASED'
                ? 'הרשמה לאירוע'
                : isFull
                  ? 'הרשמה לרשימת המתנה'
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {event.fieldsSchema.map((field: any) => {
              ...
              const hasError = isInvalid && !!fieldValidation[field.name]?.message
              return (
                <div key={field.id}>
                  <label
                    htmlFor={fieldId}
                    className={`block text-sm font-medium mb-2 ${theme.label}`}
                  >
                    {field.label}
                    {field.required && (
            ...
            {event.eventType === 'CAPACITY_BASED' &&
              (() => {
                const maxSelectable = isFull
                  ? event.maxSpotsPerPerson
                  : Math.min(event.maxSpotsPerPerson, spotsLeft)
            ...
            {/* Missing Fields Indicator — only after first submit attempt */}
            {hasAttemptedSubmit && !isFormValid && (
              <div className={`${theme.missingBg} rounded-xl p-4`} role="alert">
            ...
            <button
              type="submit"
              disabled={
                submitting || event.status !== 'OPEN' || (hasAttemptedSubmit && !isFormValid)
              }
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 p-4 shadow-2xl z-50 border-t-2 bg-white border-gray-200">
        {event.eventType === 'CAPACITY_BASED' && spotsLeft > 0 && spotsLeft < 10 && (
          <p className="text-center text-xs font-medium text-orange-600 mb-2" dir="rtl">
        ...
        <button
          ...
          disabled={submitting || event.status !== 'OPEN' || (hasAttemptedSubmit && !isFormValid)}
      ...
      {/* IS 5568 — Accessibility statement link (desktop footer) */}
      <footer className="mt-8 pb-4 text-center space-y-3 hidden md:block" dir="rtl">
      ...
      {/* Accessibility statement popup (mobile) */}
      {showAccessibility && (
        <div
        ...
        >
          <div
            ...
            lang="he"
            className="bg-white w-full max-h-[85vh] rounded-t-2xl sm:rounded-2xl sm:max-w-lg sm:mx-4 overflow-y-auto animate-slideUp"
            dir="rtl"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Handle bar */}
            <div className="sticky top-0 bg-white pt-3 pb-2 px-6 border-b border-gray-100 flex items-center justify-between">
              <h2 id="accessibility-dialog-title" className="text-lg font-bold text-gray-900">
                הצהרת נגישות
              </h2>
              <button
                onClick={() => setShowAccessibility(false)}
                className="w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 text-gray-500"
                aria-label="סגור חלון הצהרת נגישות"
              >
            ...
            <div className="px-6 py-5 space-y-5 text-sm text-gray-700 leading-relaxed">
              <section>
                ...
                <p>
                  kartis.info מחויבת להנגשת שירותיה הדיגיטליים לכלל האוכלוסייה, לרבות אנשים עם
                  מוגבלויות, בהתאם לחוק שוויון זכויות לאנשים עם מוגבלות, תשנ&quot;ח–1998, ותקנות
                  הנגישות לשירות מתשע&quot;ג–2013. האתר עומד בתקן הישראלי IS 5568 ברמת AA.
                </p>
              </section>

              <section>
                <h3 className="font-semibold text-gray-900 mb-2">אמצעי נגישות באתר</h3>
                <ul className="list-disc list-inside space-y-1">
                  ...
                  <li>ניגודיות צבעים בגובה 4.5:1 לפחות</li>
                </ul>
              </section>

              <section className="bg-gray-50 rounded-lg p-4">
                <h3 className="font-semibold text-gray-900 mb-2">פנייה בנושא נגישות</h3>
                <p>
                  דוא&quot;ל:{' '}
                  <a
